...
const books = [
  ...
  },
  {
    ...
    price: 20,
    imageSrc:
      'https://encrypted-tbn3.gstatic.com/shopping?q=tbn:ANd9GcQhis7OF8oiFvG54iVi_9uvP6jaWu57v4OZVWLwlo-9SZ0MaagOH0NZMoBuIgeWH2HxUkuLS4288szuCE9ph2uQ0qSLIROUcgvBPw66awPq&usqp=CAc',
  },
  {
    title: 'Clean Coder',
    ...
    price: 15,
    imageSrc:
      'https://encrypted-tbn3.gstatic.com/shopping?q=tbn:ANd9GcSs2gaNT_y9_TWDcBfDlI57xH31MlqTRpDYGumAKwykVYDgoEvpSEJfaq9B85DggMp9fRdxz96qQQ_KBUuZ1nSF1Aemg9H6-wF0qSKCDXV_GtpXiJ2fO9lS&usqp=CAc',
  },
  {
...
function addBookToList(book) {
  ...
  //create image and set set source
...
// *********** the bottom section of here belongs to me **********

const mainContainer = document.getElementById('main');

const selectElm = document.createElement('select');
const orderLabel = document.createElement('span');
orderLabel.textContent = 'Ordering:';
const priceLabel = document.createElement('span');
priceLabel.textContent = 'Total Price:';
const priceElm = document.createElement('span');
priceElm.textContent = '00$';
const cBoxLabel = document.createElement('label');
cBoxLabel.textContent = 'Only Read Books';
const onlyReadBooksCheckBox = document.createElement('input');
onlyReadBooksCheckBox.type = 'checkbox';
cBoxLabel.appendChild(onlyReadBooksCheckBox);

mainContainer.appendChild(orderLabel);
mainContainer.appendChild(selectElm);
mainContainer.appendChild(document.createElement('br'));
mainContainer.appendChild(priceLabel);
mainContainer.appendChild(priceElm);
mainContainer.appendChild(document.createElement('br'));
mainContainer.appendChild(cBoxLabel);
mainContainer.appendChild(bookList);

const option = document.createElement('option');
option.text = 'Select a book';
option.value = -1; // select -1 is inside the options but not a real option, it is just select a book state
...
const calculatePrice = function(bookToCalculate) {
  return bookToCalculate.reduce((total, book) => total + book.price, 0);
};

priceElm.textContent = calculatePrice(books) + '$'; // For all books price..

onlyReadBooksCheckBox.addEventListener('click', function() {
  bookList.innerHTML = ''; // firstly clean everything about book list
  if (onlyReadBooksCheckBox.checked) {
    books.filter(book => book.alreadyRead).forEach(book => addBookToList(book));
    priceElm.textContent = calculatePrice(books.filter(book => book.alreadyRead)) + '$';
  } else {
    books.forEach(book => addBookToList(book));
    priceElm.textContent = calculatePrice(books) + '$';
  }
});
...
selectElm.addEventListener('change', function(e) {
  ...
  if (Number.parseInt(this.value) >= 0) {
    // if you don't select -1
    addBookToList(books[this.value]); // create selected value
    priceElm.textContent = calculatePrice([books[this.value]]) + '$'; // create selected value's price
  } else {
    // if you select -1
    books.forEach(book => addBookToList(book));
    priceElm.textContent = calculatePrice(books) + '$';
